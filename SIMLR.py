...
def Input_SeqQual(Par):
    ...
    for line in f:
        if line_index>0:
           ...
           #print(len(list(map(float,qualarray[3:]))))
           #if position!=int(qualarray[0]):
           #   Qual_dict[str(position)]=pos_qual
           #   pos_qual={}
           #   position=int(qualarray[0])
           ...
           #error_profile_line=Qual_Substitute(ord(qualarray[1]),change)
